        ...
        private void refreshACMD_MCSInfoList(List<ACMD_MCS> currentExcuteTranCmd)
        {
            try
            {
                ...
                foreach (string new_cmd in new_add_mcs_cmds)
                {
                    ...
                    CMD_MCSObjToShow new_cmd_obj = new CMD_MCSObjToShow(scApp.VehicleBLL, cmd_obj);
        ...
        private async void btn_continuous_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    if (noticeCar.IsObstacle)
                    {
                        string msg = $"vh:{noticeCar.VEHICLE_ID}目前障礙物感測器觸發，無法下達[Continue]命令。";
            ...
            {
                sc.App.SystemParameter.setIsPassObstacleFlagWhenSendContinueRequest(false);
