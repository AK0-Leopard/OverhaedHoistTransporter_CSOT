...
namespace com.mirle.ibg3k0.sc.Scheduler
{
    public class DBManatainScheduler : IJob
    {
        ...
        private void DeleteAMCSReportQueue()
        {
            var mcs_report_queues = scApp.ReportBLL.loadBefore10Min();
            if (mcs_report_queues != null && mcs_report_queues.Count > 0)
            {
                scApp.ReportBLL.RemoteMcsReportQueueByBatch(mcs_report_queues); ;
            }
        }
