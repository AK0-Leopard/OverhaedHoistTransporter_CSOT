...
namespace com.mirle.ibg3k0.sc.WebAPI
{
    public class ReserveModule : IReserveModule
    {
        ...
        {
            scApp = _scApp;
            string s_grpc_client_ip = scApp.getString("ReservegRPCClientIP", "127.0.0.1");
            string s_grpc_client_port = scApp.getString("ReservegRPCClientPort", "25000");
                                               ...
                                               HltDirection sensorDir, HltDirection forkDir)
        {
            try
            {
                var reserve_vh_info = GetVehicleInfo(vhID, currentSectionID, vehicleX, vehicleY, vehicleAngle, speedMmPerSecond, sensorDir, forkDir);
        ...
        private Mirle.Protos.ReserveModule.VehicleInfo GetVehicleInfo(string vhID, string currentSectionID, double vehicleX, double vehicleY, float vehicleAngle, double speedMmPerSecond,
                                         HltDirection sensorDir, HltDirection forkDir)
        {
            var hlt_vh = new Mirle.Protos.ReserveModule.VehicleInfo()
            {
                VehicleId = vhID,
                VehicleX = vehicleX,
                VehicleY = vehicleY,
                ...
                ForkDirection = forkDir.convert2Direction(),
                SensorDirection = sensorDir.convert2Direction(),
                SpeedMmPreSecond = speedMmPerSecond
            };
            return hlt_vh;
        }
