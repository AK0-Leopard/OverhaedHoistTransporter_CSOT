...
namespace com.mirle.ibg3k0.sc
{
    public partial class ALINE : BaseEQObject, IAlarmHisList
    {
        ...
        [BaseElement(NonChangeFromOtherVO = true)]
        public virtual bool HasSpecifySeriousAlarmHappend
        {
        ...
        public virtual SCAppConstants.LineHostControlState.HostControlState Host_Control_State
        {
            ...
            set
            {
                if (host_control_state != value)
                {
                    ...
                    setFourColorLightByHostControlMode();
                }
            }
        }
        ...
        public bool RequestToPause(ReportBLL reportBLL, string pausrReason)
        {
            try
            {
                if (TSC_state_machine.CanFire(TSCTrigger.RequestPause))//檢查當前狀態能否進行這個Trigger
                {
                    ...
                    reportBLL.ReportTSCPauseInitiated(pausrReason);
                    //setFourColorLight();
                    return true;
                }
                else
        ...
        private void setFourColorLight()
        {
            AEQPT fourColorLight = SCApplication.getInstance().getEQObjCacheManager().getEquipmentByEQPTID("FOUR_COLOR_LIGHT");
            if (fourColorLight != null)
            {
                if (TSC_state_machine.State == TSCState.PAUSED)
                {
                    Task.Run(() => fourColorLight.setFourColorLightOrange(true));
                    Task.Run(() => fourColorLight.setFourColorLightGreen(false));
                }
                else if (TSC_state_machine.State == TSCState.AUTO)
                {
                    Task.Run(() => fourColorLight.setFourColorLightOrange(false));
                    Task.Run(() => fourColorLight.setFourColorLightGreen(true));
                }
                else
                {
                    Task.Run(() => fourColorLight.setFourColorLightOrange(false));
        ...
        public void setFourColorLightByHostControlMode()
        {
            AEQPT fourColorLight = SCApplication.getInstance().getEQObjCacheManager().getEquipmentByEQPTID("FOUR_COLOR_LIGHT");
            if (fourColorLight != null)
            {
                if (Host_Control_State == SCAppConstants.LineHostControlState.HostControlState.On_Line_Remote)
                {
                    Task.Run(() => fourColorLight.setFourColorLightOrange(false));
                    Task.Run(() => fourColorLight.setFourColorLightGreen(true));
                }
