...
namespace com.mirle.ibg3k0.sc
{
    public partial class ASEGMENT
    {
        ...
        public void SetCVID(EQObjCacheManager eQObjCacheManager)
        {
            var ohcv = eQObjCacheManager.getAllEquipment().Where(cv => (cv is OHCV) && SCUtility.isMatche((cv as OHCV).SegmentLocation, SEG_NUM)).FirstOrDefault();
            if (ohcv != null)
                CVID = ohcv.EQPT_ID;
        }
