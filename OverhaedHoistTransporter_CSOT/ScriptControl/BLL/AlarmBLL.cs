...
namespace com.mirle.ibg3k0.sc.BLL
{
    ...
    public class AlarmBLL
    {
        ...
        public bool hasSetSpecifySeriousAlarm((string eqID, string alarmCode) specifySeriousAlarmInfo)
        {
            int count = 0;
            lock (lock_obj_alarm)
            {
                using (DBConnection_EF con = DBConnection_EF.GetUContext())
                {
                    count = alarmDao.GetSetSpecifySeriousAlarmCount(con, specifySeriousAlarmInfo);
                }
            }
            return count != 0;
        }
