...
namespace com.mirle.iibg3k0.ids.ohxc.Data.Dao
{
    public class SectionDao
    {
        ...
        public ASECTION getByID(DataObjCacheManager cache_data, String section_id)
        {
            var query = from s in cache_data.Sections
                        where s.SEC_ID.Trim() == section_id.Trim()
                        orderby s.SEC_ID
                        select s;
            return query.FirstOrDefault();
        }
        public ASECTION getByFromToAdr(DataObjCacheManager cache_data, string from_adr, string to_adr)
        {
            if (string.IsNullOrWhiteSpace(from_adr)
                || string.IsNullOrWhiteSpace(to_adr))
            {
        ...
        public List<ASECTION> loadSecBySecIds(DataObjCacheManager cache_data, List<string> sec_ids)
        {
            var query = from sec in cache_data.Sections
                            //where sec_ids.Any(s => sec.SEC_ID.Contains(s.Trim()))
                        where sec_ids.Contains(sec.SEC_ID.Trim())
                        select sec;
            return query.ToList();
        ...
        public List<ASECTION> loadByFromOrToAdr(DataObjCacheManager cache_data, string adr)
        {
            if (string.IsNullOrWhiteSpace(adr))
            {
                return null;
        ...
        public List<ASECTION> loadByToAdrs(DataObjCacheManager cache_data, List<string> to_adrs)
        {
            if (to_adrs == null || to_adrs.Count == 0)
                return null;
            var query = from section in cache_data.Sections
                        where to_adrs.Contains(section.TO_ADR_ID.Trim())
                        orderby section.SEC_ORDER_NUM
                        select section;
            return query.ToList();
        ...
        public ASECTION getFirstSecBySegmentID(DataObjCacheManager cache_data, string seg_id)
        {
            var query = from section in cache_data.Sections
        ...
        public List<ASECTION> loadSectionsBySegmentID(DataObjCacheManager cache_data, string segment_num)
        {
            var query = from section in cache_data.Sections
