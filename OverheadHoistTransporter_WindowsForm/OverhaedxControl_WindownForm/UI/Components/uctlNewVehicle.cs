...
namespace com.mirle.ibg3k0.bc.winform.UI.Components
{
    ...
    public partial class uctlNewVehicle : UserControl
    {
        ...
        private AVEHICLE vh;
        public PictureBox PicAlarmStatus { get; private set; }
        public PictureBox PicCSTLoadStatus { get; private set; }

        protected Image ImgVehicleStatus = Resources.Vehicle__Auto_Local__OHT;
        protected E_VEHICLE_STATUS vehicleStatus;
        public virtual E_VEHICLE_STATUS VehicleStatus
        {
            ...
            protected set
            {
                ...
                {
                    vehicleStatus = value;
                    if (ohxc.winform.App.WindownApplication.ID == "OHS100" ||
                        ohxc.winform.App.WindownApplication.ID == "CSOT_T4"||
                        ohxc.winform.App.WindownApplication.ID == "Taichung"
                        )
                    {
                        switch (value)
                        {
                            case E_VEHICLE_STATUS.UNCONNECTIED: ImgVehicleStatus = Resources.Vehicle__Unconnected__OHT; break;
                            case E_VEHICLE_STATUS.POWERON: ImgVehicleStatus = Resources.Vehicle__Power_on__OHT; break;
                            case E_VEHICLE_STATUS.MANUAL: ImgVehicleStatus = Resources.Vehicle__Manual__OHT; break;
                            case E_VEHICLE_STATUS.AUTOLOCAL: ImgVehicleStatus = Resources.Vehicle__Auto_Local__OHT; break;
                            case E_VEHICLE_STATUS.AUTOREMOTE: ImgVehicleStatus = Resources.Vehicle__Auto_Remote__OHT; break;
                        }
                    }
                    else
                    {
        ...
        private Image ImgActionStatus = null;
    ...
    new Size(Resources.Alarm__Error_.Width / icon_scale, Resources.Alarm__Error_.Height / icon_scale);
        ...
        {
            if (!string.IsNullOrWhiteSpace(vh.CUR_ADR_ID))
            {
            ...
            else
            {
                switch (vh.ACT_STATUS)
                {
                    ...
                    case VHActionStatus.Commanding:
                        ...
                        if (PicAlarmStatus.Visible != isDisplayAlertIcon)
                        {
                            Adapter.Invoke((obj) =>
                            {
        ...
        private void updateVehiclePosition()
        {
            ...
            try
            {
                ...
                if (groupRails != null)
                {
                    ...
                    //        if (CurrentSecID != groupRails.Section_ID)
                    //        {
        ...
        private void refreshCSTIconPosition()
        {
            if (PicCSTLoadStatus.Visible)
            {
                ...
                PicCSTLoadStatus.Top = this.Top + (PicCSTLoadStatus.Height) - 7;
            }
        }
        ...
        private void pic_VhStatus_Paint(object sender, PaintEventArgs e)
        {
            ...
            g.DrawImage(ImgSpeedStatus,
                GetRectangle(RECTANGLE_KEY_SPEEDSTATUS, new Rectangle(0, this.pic_VhStatus.Height - (ImgSpeedStatus.Size.Height / icon_scale) - 5,
            ...
            if (ImgActionStatus != null)
                g.DrawImage(ImgActionStatus,
                    GetRectangle(RECTANGLE_KEY_ACTIONSTATS, new Rectangle((this.Width / 2) - ((ImgActionStatus.Width / icon_scale) / 2), 15,
                                                                        ImgActionStatus.Size.Height / icon_scale,
                                                                        ImgActionStatus.Size.Width / icon_scale)));
        }
        ...
        private void setVhCurrentSpeed(Graphics g)
        {
            ...
            float crt_speed_height = speed_status_height + ((ImgSpeedStatus.Size.Height / icon_scale) / 2) - (font_size.Height / 2) - 5;
        ...
        private void setVehicleNumbering(Graphics g)
        {
            ...
            g.DrawString(sNum, font_Numbering, objBrush, GetRectangle(RECTANGLE_KEY_NUMBERING, new RectangleF(pic_VhStatus.Width - (num_font_size.Width), pic_VhStatus.Height - num_font_size.Height - 8,
                                                                                                              num_font_size.Width, num_font_size.Height)));
        }
        ...
        private void _SetRailToolTip()
        {
            this.ToolTip.SetToolTip(this.pic_VhStatus,
                        "Current Adr : " + vh.CUR_ADR_ID?.Trim() + "\r\n" +
                        "Current SEC : " + vh.CUR_SEC_ID?.Trim() + "\r\n" +
        ...
        public event EventHandler<string> VehicleBeChosen;
        private void pic_VhStatus_Click(object sender, EventArgs e)
        {
            VehicleBeChosen?.Invoke(this, ID);
            // Uctl_Map.ohtc_Form.setMonitorVehicle(vh.VEHICLE_ID);
        }

        private void uctlNewVehicle_VisibleChanged(object sender, EventArgs e)
        {
            if (PicCSTLoadStatus != null && CSTLoadStatus != E_CST_LOAD_STATUS.NOTHING)
                PicCSTLoadStatus.Visible = this.Visible;
        }
    }
}
